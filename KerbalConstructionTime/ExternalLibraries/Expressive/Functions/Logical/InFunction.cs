    ...
    internal class InFunction : FunctionBase
    {
        ...
        public override object Evaluate(IExpression[] parameters, ExpressiveOptions options)
        {
            ...
            for (int i = 1; i < parameters.Length; i++)
            {
                ...
                if (Comparison.CompareUsingMostPreciseType( parameter, (parameters[i].Evaluate( Variables )), ((options & ExpressiveOptions.IgnoreCase) == ExpressiveOptions.IgnoreCase) ) == 0)
                {
                    found = true;
                    break;
                }
