...
namespace KerbalConstructionTime
{
    static class KCT_Utilities
    {
        ...
        public static double GetBuildRate(int index, BuildListVessel.ListType type, SpaceCenterConstruction KSC, bool UpgradedRate = false)
        {
            if (KSC == null) KSC = GameStates.ActiveKSC;
            double ret = 0;
            ...
            {
                ret = GameStates.TechList[index].BuildRate;
            }
            return ret;
        }
        ...
        public static List<double> BuildRatesVAB(SpaceCenterConstruction KSC)
        {
            return (KSC ?? GameStates.ActiveKSC).VABRates;
        }

        public static List<double> BuildRatesSPH(SpaceCenterConstruction KSC)
        {
            return (KSC ?? GameStates.ActiveKSC).SPHRates;
        }
        ...
        public static void ProgressBuildTime()
        {
            ...
            if (GameStates.lastUT == 0)
                GameStates.lastUT = UT;
            double UTDiff = UT - GameStates.lastUT;
            if (UTDiff > 0 && (HighLogic.LoadedSceneIsEditor || UTDiff < (TimeWarp.fetch.warpRates[TimeWarp.fetch.warpRates.Length - 1] * 2)))
            {
                foreach (SpaceCenterConstruction ksc in GameStates.KSCs)
                {
                    ...
                    if (ksc.VABList.Count > 0)
                    {
                        ...
                        {
                            buildRate = GetBuildRate(i, BuildListVessel.ListType.VAB, ksc);
                    ...
                    if (ksc.SPHList.Count > 0)
                    {
                        ...
                        {
                            buildRate = GetBuildRate(i, BuildListVessel.ListType.SPH, ksc);
                    ...
                    foreach (Recon_Rollout rr in ksc.Recon_Rollout)
                    {
                        ...
                        if (KCT_Utilities.CurrentGameIsCareer() && rr.RRType == Recon_Rollout.RolloutReconType.Rollout && rr.cost > 0)
                        {
                            int steps = 0;
                    ...
                    {
                        if (rr.RRType == Recon_Rollout.RolloutReconType.Rollback && rr.AsBuildItem().IsComplete())
                        {
                            BuildListVessel blv = KCT_Utilities.FindBLVesselByID(new Guid(rr.associatedID));
                            if (blv != null)
                                blv.launchSiteID = -1;
                        }
                    });
                    ksc.Recon_Rollout.RemoveAll(rr => !KCT_PresetManager.Instance.ActivePreset.generalSettings.ReconditioningTimes || (rr.RRType != Recon_Rollout.RolloutReconType.Rollout && rr.AsBuildItem().IsComplete()));

                    foreach (UpgradingBuilding kscTech in ksc.KSCTech)
                    {
                        ...
                        {
                            if (ScenarioUpgradeableFacilities.Instance != null && GameStates.erroredDuringOnLoad.OnLoadFinished)
                                kscTech.Upgrade();
                        }
                    }
                ...
                for (int i = 0; i < GameStates.TechList.Count; i++)
                {
                    KCT_TechItem tech = GameStates.TechList[i];
                    ...
                    {
                        if (GameStates.settings.ForceStopWarp && TimeWarp.CurrentRate > 1f)
                            TimeWarp.SetRate(0, true);
                        if (tech.protoNode == null) continue;
                        tech.EnableTech();
                        GameStates.TechList.Remove(tech);
                        if (KCT_PresetManager.PresetLoaded() && KCT_PresetManager.Instance.ActivePreset.generalSettings.TechUpgrades)
                            GameStates.MiscellaneousTempUpgrades++;

                        for (int j = 0; j < GameStates.TechList.Count; j++)
                            GameStates.TechList[j].UpdateBuildRate(j);
                    }
                }
            }
            if (GameStates.targetedItem != null && GameStates.targetedItem.IsComplete())
            {
                TimeWarp.SetRate(0, true);
        ...
        public static String GetButtonTexture()
        {
            ...
            if (!KCT_PresetManager.Instance.ActivePreset.generalSettings.Enabled)
                return PluginAssemblyUtilities.iconKCTOff;

            //Flash for up to 3 seconds, at half second intervals per icon
            if (GameStates.kctToolbarButton.Important && (DateTime.Now.CompareTo( startedFlashing.AddSeconds( 3 ) )) < 0 && DateTime.Now.Millisecond < 500)
                textureReturn = PluginAssemblyUtilities.iconKCTOff;
            //If it's been longer than 3 seconds, set Important to false and stop flashing
            else if (GameStates.kctToolbarButton.Important && (DateTime.Now.CompareTo( startedFlashing.AddSeconds( 3 ) )) > 0)
            {
                GameStates.kctToolbarButton.Important = false;
                textureReturn = PluginAssemblyUtilities.iconKCTOn;
            }
            //The normal icon
            else
                textureReturn = PluginAssemblyUtilities.iconKCTOn;
        ...
        public static void MoveVesselToWarehouse(int ListIdentifier, int index, SpaceCenterConstruction KSC)
        {
            if (KSC == null) KSC = GameStates.ActiveKSC;
            if (GameStates.kctToolbarButton != null)
            {
                GameStates.kctToolbarButton.Important = true; //Show the button if it is hidden away
            ...
            if ((GameStates.settings.ForceStopWarp || vessel == GameStates.targetedItem) && TimeWarp.CurrentRateIndex != 0)
            {
                TimeWarp.SetRate(0, true);
                GameStates.warpInitiated = false;
            }
            ...
            Log.Trace("Moved vessel " + vessel.shipName + " to " +KSC.KSCName + "'s " + stor + " storage.");
            ...
            if (!GameStates.settings.DisableAllMessages)
            {
        ...
        public static double SpendFunds(double toSpend, TransactionReasons reason)
        {
            if (!CurrentGameIsCareer())
                return 0;
            Log.Trace("Removing funds: " + toSpend + ", New total: " + (Funding.Instance.Funds - toSpend));
        ...
        public static double AddFunds(double toAdd, TransactionReasons reason)
        {
            if (!CurrentGameIsCareer())
                return 0;
            Log.Trace("Adding funds: " + toAdd + ", New total: " + (Funding.Instance.Funds + toAdd));
        ...
        public static BuildListVessel AddVesselToBuildList(string launchSite)
        {
            ...
            BuildListVessel blv = new BuildListVessel(EditorLogic.fetch.ship, launchSite, GetBuildTime(EditorLogic.fetch.ship.Parts), EditorLogic.FlagURL);
        ...
        public static BuildListVessel AddVesselToBuildList(BuildListVessel blv)
        {
            if (CurrentGameIsCareer())
            {
                ...
                {
                    Log.Trace("Tried to add " + blv.shipName + " to build list but not enough funds.");
                    Log.Trace("Vessel cost: " + GetTotalVesselCost(blv.shipNode) + ", Current funds: " + prevFunds);
            ...
            Log.Trace("Added " + blv.shipName + " to " + type + " build list at KSC "+GameStates.ActiveKSC.KSCName+". Cost: "+blv.cost);
            Log.Trace("Launch site is " + blv.launchSite);
            //Log.Trace("Cost Breakdown (total, parts, fuel): " + blv.totalCost + ", " + blv.dryCost + ", " + blv.fuelCost);
        ...
        public static IKCTBuildItem NextThingToFinish()
        {
            ...
            foreach (IKCTBuildItem blv in GameStates.TechList)
            {
                double time = blv.GetTimeLeft();
                if (time < shortestTime)
                {
        ...
        public static void RampUpWarp(IKCTBuildItem item)
        {
            ...
            while ((newRate + 1 < TimeWarp.fetch.warpRates.Length) && (timeLeft > TimeWarp.fetch.warpRates[newRate + 1]*Planetarium.fetch.fixedDeltaTime) && (newRate < GameStates.settings.MaxTimeWarp))
            {
                newRate++;
            }
        ...
        public static int TotalSpentUpgrades(SpaceCenterConstruction ksc)
        {
            if (ksc == null) ksc = GameStates.ActiveKSC;
            int spentPoints = 0;
        ...
        public static bool KSCSwitcherInstalled
        {
            get
            {
                if (_KSCSwitcherInstalled == null)
                {
                    ...
                    //Log.Trace("KSCSwitcher status: " + _KSCSwitcherInstalled);
                }
        ...
        public static void SetActiveKSCToRSS()
        {
            ...
            {
                Log.Trace("Setting active site to " + site);
        ...
        {
            if (site == "") site = "Stock";
            if (GameStates.ActiveKSC == null || site != GameStates.ActiveKSC.KSCName)
            {
                Log.Trace("Setting active site to " + site);
                SpaceCenterConstruction setActive = GameStates.KSCs.FirstOrDefault(ksc => ksc.KSCName == site);
                if (setActive != null)
                {
                    GameStates.ActiveKSC = setActive;
                }
                else
                {
                    setActive = new SpaceCenterConstruction(site);
                    if (CurrentGameIsCareer())
                        setActive.ActiveLPInstance.level = 0;
                    GameStates.KSCs.Add(setActive);
                    GameStates.ActiveKSC = setActive;
                }
            }
            GameStates.activeKSCName = site;
        }
        ...
        public static void RecalculateEditorBuildTime(ShipConstruct ship)
        {
            ...
            GameStates.EditorBuildTime = GetBuildTime(ship.Parts);
            GameStates.EditorRolloutCosts = KCT_MathParsing.ParseRolloutCostFormula(new BuildListVessel(ship, EditorLogic.fetch.launchSiteName, GameStates.EditorBuildTime, EditorLogic.FlagURL));
        }
        ...
        public static bool ReconditioningActive(SpaceCenterConstruction KSC, string launchSite = "LaunchPad")
        {
            if (KSC == null) KSC = GameStates.ActiveKSC;
        ...
        {
            BuildListVessel ret = null;
            foreach (SpaceCenterConstruction ksc in GameStates.KSCs)
            {
                BuildListVessel tmp = ksc.VABList.Find(v => v.id == id);
                if (tmp != null)
                {
                    ret = tmp;
                    break;
        ...
        public static int BuildingUpgradeMaxLevel(string facilityID)
        {
            ...
            if (lvl < 0)
            {
                if (!GameStates.BuildingMaxLevelCache.TryGetValue(facilityID.Split('/').Last(), out lvl))
                {
                    //screw it, let's call it 2
                    lvl = 2;
                    Log.Trace($"Couldn't get actual max level or cached one for {facilityID}. Assuming 2.");
                }
            }
            return lvl;
        }
        ...
        public static int BuildingUpgradeMaxLevel(SpaceCenterFacility facility)
        {
            ...
            if (lvl < 0)
            {
                if (!GameStates.BuildingMaxLevelCache.TryGetValue(facility.ToString(), out lvl))
                {
                    //screw it, let's call it 2
                    lvl = 2;
                    Log.Trace($"Couldn't get actual max level or cached one for {facility}. Assuming 2.");
                }
            }
            return lvl;
        }
        ...
        public static int TotalUpgradePoints()
        {
            ...
            if (KCT_PresetManager.Instance.ActivePreset.generalSettings.TechUpgrades)
            {
                ...
                {
                    total += GameStates.LastKnownTechCount;
                    if (GameStates.LastKnownTechCount == 0)
                ...
                total += GameStates.TechList.Count;
            }
            //Purchased funds
            total += GameStates.PurchasedUpgrades[0];
            //Purchased science
            total += GameStates.PurchasedUpgrades[1];
            //Inventory sales
            total += (int)GameStates.InventorySaleUpgrades;
            //Temp upgrades (currently for when tech nodes finish)
            total += GameStates.MiscellaneousTempUpgrades;
            
            //Misc. (when API)
            total += GameStates.TemporaryModAddedUpgradesButReallyWaitForTheAPI;
            total += GameStates.PermanentModAddedUpgradesButReallyWaitForTheAPI;
        ...
        public static bool RecoverActiveVesselToStorage(BuildListVessel.ListType listType)
        {
            ...
            try
            {
                Log.Trace("Attempting to recover active vessel to storage.");
                GamePersistence.SaveGame("KCT_Backup", HighLogic.SaveFolder, SaveMode.OVERWRITE);
                GameStates.recoveredVessel = new BuildListVessel(FlightGlobals.ActiveVessel);
                GameStates.recoveredVessel.type = listType;
                if (listType == BuildListVessel.ListType.VAB)
                    GameStates.recoveredVessel.launchSite = "LaunchPad";
                else
                    GameStates.recoveredVessel.launchSite = "Runway";

                //check for symmetry parts and remove those references if they can't be found
                RemoveMissingSymmetry(GameStates.recoveredVessel.shipNode);

                //test if we can actually convert it
                bool success = test.LoadShip(GameStates.recoveredVessel.shipNode);
                if (success)
                    ShipConstruction.CreateBackup(test);
                Log.Trace("Load test reported success = " + success);
                if (!success)
                {
        ...
        public static void RemoveMissingSymmetry(ConfigNode ship)
        {
            ...
            Log.Trace("Removed " + referencesRemoved + " invalid symmetry references.");
        }
        ...
        public static void HandleEditorButton()
        {
            ...
            {
                Log.Trace("Attempting to take control of launch button");
                ...
                foreach (var v in controller.GetType().GetFields( System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.FlattenHierarchy | System.Reflection.BindingFlags.Public  ))
                {
                    Log.Trace( "Name: " + v.Name + " ; MemberType: " + v.MemberType.ToString() + " ; Module: " + v.Module + " ; Type: " + v.ReflectedType.Name);
                }
